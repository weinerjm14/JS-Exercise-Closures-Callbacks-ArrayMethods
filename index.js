 ...
 * should return "sad".
*/
function processContains(x, list, callback) {
  return callback(list.includes(x));
...
function firstNamesAllCaps(runners) {
  let firstAllCap = [];
  runners.map((runners) =>{
    return firstAllCap.push(runners.first_name.toUpperCase());
  })
  return firstAllCap;
}
...
*/
function getRunnersByTShirtSize(runners, tSize) {
    const arrTsize = runners.filter((runners) => {return runners.shirt_size === tSize});
    return arrTsize;
...
function tallyUpDonations(runners) {
  let TotDonations = runners.reduce((accum, runners) => {
    return accum + runners.donation;}, 0);
  return TotDonations;
}
